...
export const Trains: React.FC = (props) => {
  ...
  const loadData = async () => {
    if (doLoadData === true) {
      ...
      const response_trainstaions = await axios.get(`${settings.protocol}://` + settings.ip + settings.port + "/getTrainStation");
      ...
      {trains && tStation_PrevNext ? (
        <>
          {trains.map((train: any, index: number) => {
            ...
            if (tStation_PrevNext.length > 0 && tStation_PrevNext[index].length > 0) {
              left = Math.floor(
                Math.pow(
                  Math.pow(tStation_PrevNext[index][0].location.x - train.location.x, 2) +
                    Math.pow(tStation_PrevNext[index][0].location.y - train.location.y, 2),
                  0.5
                ) / 100
              );
              right = Math.floor(
                Math.pow(
                  Math.pow(tStation_PrevNext[index][1].location.x - train.location.x, 2) +
                    Math.pow(tStation_PrevNext[index][1].location.y - train.location.y, 2),
                  0.5
                ) / 100
              );
              ...
              totalLength = Math.floor(
                Math.pow(
                  Math.pow(tStation_PrevNext[index][0].location.x - tStation_PrevNext[index][1].location.x, 2) +
                    Math.pow(tStation_PrevNext[index][0].location.y - tStation_PrevNext[index][1].location.y, 2),
                  0.5
                ) / 100
              );
              ...
              <Grid container spacing={2} sx={{ marginY: "30px" }} display={"flex"} alignItems={"center"}>
                <Grid xs={3}>
                  <Card variant="outlined" sx={{ position: "relative" }}>
                    {tStation_PrevNext.length > 0 && tStation_PrevNext[index].length > 0 ? (
                      <CardContent>
                        {/* <GiCargoCrate size="36px"/> */}

                        <Stack alignItems={"center"}>
                          <img
                            src="./assets/Buildings/IconDesc_DockingStation_256.png"
                            alt="image"
                  ...
                  <Card variant="outlined" sx={{ position: "relative", overflow: "hidden" }}>
                    <CardContent>
                        ...
                        <Grid>
                          <Typography sx={{ color: "rgba(255,255,255,0.9)" }}>
                            {parseFloat(train.ForwardSpeed) < 0
                              ? (parseInt(train.ForwardSpeed) * -1) / 10
                              : parseInt(train.ForwardSpeed) / 10}{" "}
                            km/h
                          </Typography>
                        </Grid>
                      </Grid>
                      ...
                      <LinearProgress
                        ...
                        variant="soft"
                        value={percentDone}
                        sx={{ position: "absolute", bottom: "0px", left: "0px", right: "0px" }}
                      />
                    </CardContent>
                  </Card>
                </Grid>
