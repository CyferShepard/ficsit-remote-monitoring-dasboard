...
export const Drones: React.FC = (props) => {
  ...
  const loadData = async () => {
    if (doLoadData === true) {
      const response = await axios.get(`${settings.protocol}://` + settings.ip + settings.port + "/getDrone");
      const response_station = await axios.get(`${settings.protocol}://` + settings.ip + settings.port + "/getDroneStation");
        ...
        <>
          {drones.map((drone: any, index: number) => {
            ...
            if (drStation_PrevNext[index].length > 0 && drStation_PrevNext[index][0] && drStation_PrevNext[index][1]) {
              top = Math.floor(
                Math.pow(
                  Math.pow(drStation_PrevNext[index][0].location.x - drone.location.x, 2) +
                    Math.pow(drStation_PrevNext[index][0].location.y - drone.location.y, 2),
                  0.5
                ) / 100
              );
              bottom = Math.floor(
                Math.pow(
                  Math.pow(drStation_PrevNext[index][1].location.x - drone.location.x, 2) +
                    Math.pow(drStation_PrevNext[index][1].location.y - drone.location.y, 2),
                  0.5
                ) / 100
              );
              ...
              totalLength = Math.floor(
                Math.pow(
                  Math.pow(drStation_PrevNext[index][0].location.x - drStation_PrevNext[index][1].location.x, 2) +
                    Math.pow(drStation_PrevNext[index][0].location.y - drStation_PrevNext[index][1].location.y, 2),
                  0.5
                ) / 100
              );
                ...
                <Grid container spacing={2} sx={{ marginBottom: "30px", height: "300px" }} display={"flex"} alignItems={"center"}>
                  <Grid xs={3} sx={{ height: "240px" }}>
                    {drStation_PrevNext[index][0] ? (
                      <Card
                        variant="outlined"
                        sx={{ position: "relative", marginBottom: "20px", height: "110px", paddingBottom: "0px !important" }}
                      >
                        <CardContent>
                    ...
                    <Card variant="outlined" sx={{ position: "relative", height: "255px", paddingTop: 0 }}>
                      <CardContent>
                        ...
                        <Grid container>
                          ...
                          <Grid>
                            <Typography sx={{ color: "rgba(255,255,255,0.9)" }}>
                              {parseFloat(drone.FlyingSpeed) < 0 ? parseInt(drone.FlyingSpeed) * -1 : parseInt(drone.FlyingSpeed)}{" "}
                              Knots
                            </Typography>
                          </Grid>
                        </Grid>
                        <Grid container>
                          ...
                          <Grid>
                            <Typography sx={{ color: "rgba(255,255,255,0.9)" }}>
                              {(drone.location.z / 1000).toFixed(2)} m
                            </Typography>
                          </Grid>
                        </Grid>
                      </CardContent>
